...
/* ---------- CORS ---------- */
const allowedOrigins = (process.env.ALLOWED_ORIGINS || 'http://localhost:3001').split(',').map(s => s.trim());
app.use(cors({
...
app.post('/api/submit', async (req, res) => {
  try {
    ...
    // Add metadata
    data.source = req.headers.origin || req.headers.referer || 'direct';
    data.submittedAt = new Date().toISOString();

    const subjects = {
      vendor: '[AARAA Website] New Vendor Registration',
      enquiry: '[AARAA Website] New Quick Enquiry',
      enquiry_bottom: '[AARAA Website] New Project Enquiry'
    };

    const subject = subjects[formType] || '[AARAA Website] New Form Submission';
    const html = buildEmailHTML(data, formType);
    const recipient = process.env.RECIPIENT_EMAIL || 'info@aaraainfrastructure.com';

    const mailOptions = {
      from: `"AARAA Website" <${process.env.EMAIL_USER || 'info@aaraainfrastructure.com'}>`,
      to: recipient,
      subject: subject,
      html: html
    };
